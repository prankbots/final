...
class Talk(object):
    ...
    def likePost(self,mid,postid,likeType=1001):
        ...
        r = requests.post(
          self.LINE_TIMELINE_API+"/v23/like/create.json?homeId="+mid,
          headers=self.channelHeaders,
    ...
    def unlikePost(self,mid,postId):
      ...
      r=requests.post(
        self.LINE_TIMELINE_API+"/v23/like/delete.json?homeId="+mid,
        headers=self.channelHeaders,
        data=json.dumps(payload)
      )
      return r.json()
    ...
    @loggedIn
    def sendContactWithFooter(self, to, mid, link, icon, footer):
        contentMetadata = {'mid': mid}
        return self.sendMessageFooter(to, '',mid, link, icon, footer)
    ...
    @loggedIn
    def reMessage(self, relatedMessageId, to, text, contentMetadata={}, contentType=0):
    ...
    @loggedIn
    def sendCContact(self, to, mid):
        objectId = self.sendMessage(to=to, text=None, contentMetadata = 13)
        return self.uploadObjTalk(path=path, type='mid', returnAs='bool', objld=objectId)
